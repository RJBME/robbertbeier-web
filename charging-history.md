...
<style>
  ...
  .filter-bar { display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 12px; background: var(--dash-card); padding: 20px; border-radius: 12px; margin-bottom: 25px; border: 1px solid var(--dash-border); align-items: end; }
...
<script>
// Auto-populate filters based on data
function initFilters() {
  ...
  Array.from(locations).sort().forEach(l => { locSelect.add(new Option(l, l.toLowerCase())); });
  
  const vehSelect = document.getElementById('vehFilter');
  Array.from(vehicles).sort().forEach(v => { vehSelect.add(new Option(v, v.toLowerCase())); });
}
...
function applyFilters() {
  ...
  document.querySelectorAll('.log-row').forEach(row => {
    ...
    const matchLoc = !loc || row.getAttribute('data-loc').toLowerCase().includes(loc);
    const matchVeh = !veh || row.getAttribute('data-veh').toLowerCase().includes(veh);
