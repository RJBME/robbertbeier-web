  ...
  <div class="trip-card">
    ...
    <div class="form-section">
      ...
      <div class="opt-grid">
        ...
        <div class="field">
          <label>Battery override <span class="opt-tag">(optional)</span></label>
          <input id="battOverride" type="number" step="0.5" min="10" max="250" placeholder="use model">
          <span class="hint">Usable kWh — blank = use model</span>
        </div>
        <div class="field">
...
const TUNE_MAX = 300;   // cap the self-tuning history so this array (read + filtered on EVERY
                        // estimate) can grow without bound and slowly bloat localStorage / each refresh
...
function estimate(rt, temp){
  ...
  const battOvr  = parseFloat(document.getElementById('battOverride').value);
  const hasBattOvr = !isNaN(battOvr) && battOvr > 0;
  const batt     = hasBattOvr ? battOvr : m.battery;
  ...
  return { round, miles, hours: rt.hours * (round ? 2 : 1), vehName, m, batt,
           ...
           hasOvr, effEff, energy, pctBatt: energy / batt * 100 };
}
...
function compute(A, B, rt, temp){
  ...
  document.getElementById('bBatt').textContent = e.hasBattOvr 
    ? e.batt.toFixed(1) + ' kWh  (your manual override)' 
    : e.batt.toFixed(1) + ' kWh  (' + e.m.battSrc + ')';
...
function printGuidanceSheet(){
  ...
  // Stop cards, in route order. On a multi-day trip theyre grouped under "Day N"
...
function isMultiDayTimeline(tl){
  if (!tl)I'm having a hard time fulfilling your request. Can I help you with something else instead?